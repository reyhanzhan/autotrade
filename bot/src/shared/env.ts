...
const schema = z.object({
  ...
  MAX_ENTRY_SPREAD_PCT: z.coerce.number().min(0).max(5).default(0.05),
  MAX_SAME_SIDE_POSITIONS: z.coerce.number().int().min(1).max(20).default(2),
  INTERVAL: z
  ...
  DYNAMIC_CONFIDENCE_ENABLED: truthy.default("true"),
  ...
  AUTO_BLACKLIST_LOOKBACK_DAYS: z.coerce.number().int().min(1).max(90).default(7),
  AUTO_BLACKLIST_DAYS: z.coerce.number().int().min(1).max(90).default(14),
  AUTO_BLACKLIST_MIN_TRADES: z.coerce.number().int().min(1).max(50).default(3),
  AUTO_BLACKLIST_MAX_WIN_RATE: z.coerce.number().min(0).max(1).default(0.3),
  AUTO_BLACKLIST_LOSS_STREAK: z.coerce.number().int().min(1).max(20).default(3),
  BTC_REGIME_ENABLED: truthy.default("true"),
  BTC_REGIME_CACHE_MS: z.coerce.number().int().min(10_000).max(900_000).default(60_000),
  BTC_CRASH_1H_PCT: z.coerce.number().min(0.1).max(50).default(2),
  BTC_CRASH_4H_PCT: z.coerce.number().min(0.1).max(50).default(4),
  BTC_RANGING_ADX: z.coerce.number().min(1).max(100).default(18),
  BTC_RANGING_ATR_PCT: z.coerce.number().min(0.01).max(10).default(0.8),
